# English Quality Comparison Audio

This folder stores a new English-only quality-comparison bundle without modifying the older demo assets.
...
Rows used in the quality-comparison tables:
- `teacher_*`: 10-step teacher
- `baseline_*`: 1-step degraded teacher baseline with CFG = 0.0
- `baseline8_*`: 8-step teacher baseline with CFG = 0.0
- `cfg_w*.wav`: 8-step teacher baseline with varying CFG values for the CFG and ablation sections
